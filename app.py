...
with st.sidebar:
    ...
    st.markdown("01 · Resume parsing")
    st.markdown("02 · Requirement extraction")
    st.markdown("03 · Skill matching")
    st.markdown("04 · Semantic matching")
    st.markdown("05 · Evidence explanation")
    st.divider()
...
with left:
    ...
    resume_file = st.file_uploader(
        "Upload PDF or DOCX",
        type=["pdf", "docx"],
        help="The resume is parsed locally by the application.",
    )
    ...
    resume_years = st.number_input(
        ...
        value=0.0,
        step=0.5,
        help="Optional. Set 0 if the candidate is a fresher.",
    )
    ...
    if resume_file is None:
        ...
        try:
            with st.spinner("Parsing, embedding and matching…"):
                ...
                st.session_state.explanation = generate_explanation(result_dict)
...
<div class="score-card">
    ...
    <div class="score-label">OVERALL MATCH / 100</div>
</div>
""",
            unsafe_allow_html=True,
        )
    ...
    with skills_col:
        ...
        html = "".join(f'<span class="pill">✓ {skill}</span>' for skill in matched)
        st.markdown(
            f'<div class="card"><strong>Matched skills</strong><br>{html or "<span class=\"small-muted\">No configured skills detected.</span>"}</div>',
            unsafe_allow_html=True,
        )

    with gaps_col:
        missing = match["missing_required_skills"]
        html = "".join(f'<span class="pill pill-missing">! {skill}</span>' for skill in missing)
        st.markdown(
            f'<div class="card"><strong>Missing required skills</strong><br>{html or "<span class=\"small-muted\">None detected.</span>"}</div>',
            unsafe_allow_html=True,
        )
    ...
    st.markdown(
        f'<div class="card"><span class="small-muted">Generated by {source}</span><br><br>{str(explanation.get("text", "")).replace(chr(10), "<br><br>")}</div>',
        unsafe_allow_html=True,
    )
    ...
    download_payload = json.dumps(result, indent=2)
    st.download_button(
